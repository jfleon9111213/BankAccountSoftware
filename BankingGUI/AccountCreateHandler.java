...
public class AccountCreateHandler extends BaseHandler{
    ...
    private void createFields() {
        ...
        textPin.textProperty().addListener((source, oldValue, newValue) -> {
            if(newValue.length() > 4 || !(newValue.matches("^[0-9]*$"))) {
                textPin.setText(oldValue);
            }
        });
    }
    ...
    private void createButtons() {
        ...
        signUp.setOnAction(event -> {
            if (textPin.getText().length() < 4 || emptyFields()) {
                Alert signUpFail = new Alert(Alert.AlertType.ERROR, "Field was not fully filled out.");
                signUpFail.setHeaderText("Sign up failed.");
                signUpFail.show();
            }
    ...
    private boolean emptyFields() {
        return textFirst.getText().isEmpty() || textLast.getText().isEmpty() || textUser.getText().isEmpty() || textPass.getText().isEmpty()
                || choiceMonth.getSelectionModel().isEmpty() 
                || choiceDay.getSelectionModel().isEmpty() 
                || choiceYear.getSelectionModel().isEmpty();
